<?php
/**
 * 📧 Sistema de envío de emails con SMTP nativo - LUMETRIX
 * Versión mejorada que usa SMTP directo de Hostalia (sin librerías externas)
 * ⚡ MUCHO MÁS RÁPIDO que mail() nativo de PHP
 */

require_once __DIR__ . '/config_smtp.php';

/**
 * Enviar email de verificación con SMTP
 */
function enviarEmailVerificacion($email, $nombre, $codigo) {
    ...
    try {
        return enviarEmailSMTP($email, $asunto, $html);
    } catch (Exception $e) {
...
/**
 * 🚀 ENVIAR EMAIL VÍA SMTP (RÁPIDO Y CONFIABLE)
 * Usa sockets nativos de PHP, no requiere librerías externas
 */
function enviarEmailSMTP($to, $subject, $htmlBody) {
    if (SMTP_DEBUG) {
        error_log("📧 Iniciando envío SMTP a: $to");
    }
    ...
    $headers .= "X-Mailer: LUMETRIX-SMTP/1.0\r\n";
    $headers .= "Date: " . date('r') . "\r\n";
    $headers .= "\r\n";
    ...
    if (strpos($response, '250') !== false) {
        if (SMTP_DEBUG) error_log("✅ Email enviado correctamente a: $to");
        return true;
    } else {
