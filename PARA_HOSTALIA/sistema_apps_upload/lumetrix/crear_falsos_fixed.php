...
echo "<h2>🎮 Creando jugadores falsos (versión corregida)</h2>";
...
try {
    ...
    foreach ($jugadores as $j) {
        ...
        // Insertar usuario (incluyendo el campo 'nombre' que faltaba)
        $sql1 = "INSERT INTO usuarios_aplicaciones (usuario_aplicacion_key, email, nombre, nick, password_hash, app_codigo, fecha_registro) VALUES (?, ?, ?, ?, ?, 'lumetrix', NOW())";
        $st1 = $pdo->prepare($sql1);
        $st1->execute([$uakey, $j['email'], $j['nick'], $j['nick'], password_hash('123', PASSWORD_DEFAULT)]);
