 ...
 *
 * Fix en caliente para pantalla en blanco:
 *  - Código estaba incompleto (refs/funciones sin definir, vars fantasma: ACCENT_HUE, placed, seqRef...).
 *  - Restauro app COMPLETA y funcional (Intro + Game + Modales) manteniendo estilo neón.
 *  - Melodías de 15 notas aleatorias por nivel; al ganar suena la melodía completa.
 *  - Color de acierto = color del borde del nivel.
 *  - Delegación de eventos estable (pointerdown) + pista inicial con retardo.
 *  - Debug: selector de nivel en Opciones.
 */
...
function tilesFor(level){
  ...
  // Mundo 1: [4,4,5,5,6,6,7,8,9,9]
  const M1 = [4,4,5,5,6,6,7,8,9,9];
  // Mundo 2: +1 cada uno (5..10) — dejamos la méca de drag para >=11 pero sin bloquear taps normales.
  const M2 = M1.map(n=>n+1);
...
const MELODIES = [
  ...
  [262,294,330,392,494,523,494,392,330,294,262,196,220,247,294],
  [220,247,262,294,330,349,392,440,392,349,330,294,262,247,220],
  [262,330,392,523,392,330,262,330,392,523,659,523,392,330,262],
  [392,392,440,392,523,494,392,392,440,392,587,523,392,392,392]
];

// ---------------- Sonido (WebAudio) ----------------
function useSFX(enabled){
  const ctxRef = useRef(null);
  const getCtx = ()=>{ if(!enabled) return null; try{ if(!ctxRef.current){ const C=window.AudioContext||window.webkitAudioContext; ctxRef.current=new C(); } return ctxRef.current; }catch{return null;} };
  const tone = (f=440,d=0.12,type='sine',gain=0.07)=>{ const ctx=getCtx(); if(!ctx) return; const o=ctx.createOscillator(); const g=ctx.createGain(); o.type=type; o.frequency.value=f; g.gain.value=gain; o.connect(g); g.connect(ctx.destination); o.start(); o.stop(ctx.currentTime+d); };
  const playMelody = (arr,dur=0.12,gap=0.04)=>{ const ctx=getCtx(); if(!ctx||!arr||!arr.length) return; arr.forEach((f,i)=>{ const o=ctx.createOscillator(); const g=ctx.createGain(); o.type='triangle'; o.frequency.value=f; g.gain.value=0.08; o.connect(g); g.connect(ctx.destination); const t=ctx.currentTime + i*(dur+gap); o.start(t); o.stop(t+dur); }); };
  return {
    ...
    winMelody: (arr)=>playMelody(arr && arr.length?arr:[659.25,880.0,1046.5], 0.12, 0.04),
  };
}
...
// ---------------- Game ----------------
function Game({ level, setLevel, soundOn, vibrateOn, onOpenAuth, onOpenRanking, onOpenOptions }){
  ...
  // aplicar color al borde
  useEffect(()=>{ const root=boardRef.current?.closest('.device'); if(root) root.style.setProperty('--accent', accent); },[accent]);

  // helpers de tiempo
  const saveTotal=(secs)=>{ try{ const prev=Number(JSON.parse(localStorage.getItem('lum_total')||'0'))||0; const next=prev+secs; localStorage.setItem('lum_total', JSON.stringify(next)); setTotalTime(next); }catch{} };
  ...
  function placeTiles(n){
    ...
    for(let i=0;i<n;i++){
      ...
      // Evita arrancar con el mismo color exacto del borde
      if (b.style.background === (paintRef.current || accent)) {
        const alt = (ACCENT_HUE[paintRef.current || accent] + 180) % 360;
  ...
  function start(){
    ...
    const t=timeFor(level); setTime(t); setRunning(true); runningRef.current=true; SFX.start(); startTimeRef.current=Date.now();
    const t0=Date.now();
    timerRef.current=setInterval(()=>{
      const el=(Date.now()-t0)/1000; const rem=Math.max(0,t-el);
      setTime(Math.ceil(rem));
      if(rem<=0){ const spent=Math.ceil((Date.now()-startTimeRef.current)/1000); saveTotal(spent); clearInterval(timerRef.current); setRunning(false); runningRef.current=false; setLose(true); SFX.fail(); resetTiles(); }
    },100);

    setTimeout(hint, 700); // pista con retardo
  }

  function tap(id){
    if(!runningRef.current) return; const expected=seqRef.current[stepRef.current];
    const board=boardRef.current;
    if(id===expected){ const el=board && board.querySelector(`.tile[data-id="${id}"]`); if(el){ const pitch = parseFloat(el.dataset.pitch||'880'); el.style.background = (paintRef.current || accent); el.classList.remove('lit'); SFX.ok(pitch); vibrate(20, vibrateOn); } stepRef.current++; if(stepRef.current>=seqRef.current.length){ const spent=Math.ceil((Date.now()-startTimeRef.current)/1000); saveTotal(spent); setRunning(false); runningRef.current=false; if(timerRef.current) clearInterval(timerRef.current); setWin(true); SFX.winMelody((melodyRef.current && melodyRef.current.length) ? melodyRef.current.slice(0, 6) : [659.25,880.0,1046.5,880.0,659.25,523.25]); } }
    else { const spent=Math.ceil((Date.now()-startTimeRef.current)/1000); saveTotal(spent); SFX.fail(); vibrate(80, vibrateOn); stepRef.current=0; resetTiles(); startTimeRef.current=Date.now(); }
  }

  function nextLevel(){ setWin(false); setLose(false); setLevel(l=>l+1); setTimeout(start, 0); }
    ...
    <section className="screen">
      ...
      <div className="hud">
        ...
        <div className="meta">
          ...
          <span className="chip">Σ<b>{totalTime}s</b></span>
        </div>
      </div>
      <div className="board" ref={boardRef}>
        {!running && !win && !lose && (<div className="overlay"><button className="btn btn1" onClick={start}>Empezar</button></div>)}
        {win && (<div className="overlay"><div className="card" style={{textAlign:'center'}}><h3 style={{color:'var(--neon2)'}}>¡Nivel superado!</h3><button className="btn btn1" onClick={nextLevel}>Siguiente</button></div></div>)}
        {lose && (<div className="overlay"><div className="card" style={{textAlign:'center'}}><h3 style={{color:'var(--neon1)'}}>Tiempo agotado</h3><button className="btn btn1" onClick={start}>Reintentar</button></div></div>)}
...
function Ranking({ onClose, total }){
  return (
    <div className="modal"><div className="card">
      ...
      <h3 style={{ color: 'var(--neon2)', marginTop:0 }}>Ranking (placeholder)</h3>
      <div className="list"><div>Tu tiempo acumulado: <b>{total}s</b></div></div>
    </div></div>
  );
}
function Options({ onClose, onOpenAuth, level, setLevel, soundOn, vibrateOn, setSoundOn, setVibrateOn }){
  return (
    <div className="modal"><div className="card">
      <button className="closer" onClick={onClose}>✕</button>
